...
CREATE TABLE FACTURAS( 
   ...
   FECHA_DE_PAGO               DATE    NOT NULL,        -- FECHA LIMITE DE PAGO CON PRECIO NORMAL
...
SELECT
	  ...
	  FACTURA_EMITIDA,
      FECHA_DE_PAGO           AS Fecha_De_Pago,
      DATEADD(day, DATEDIFF(day, FECHA_DE_PAGO, FECHA_DE_CANCELACION) * 0.98, FACTURA_EMITIDA) AS Monto_Pago_Tardio_Movil
FROM  Departamento a,
      Agencias b,
      Clientes c,
...
WHERE FECHA_DE_CANCELACION< GETDATE()
  AND c.saldo > 0
...
-- SELECT*FROM PAGOS_TARDIOS_CLIENTES
-- DROP VIEW PAGOS_TARDIOS_CLIENTES
...
SELECT PFC.ID_PAGO, PFC.ID_FACTURA, PFC.ID_CLIENTE, PFC.ID_FECHA, PFC.MONTO_DE_FACTURA, PFC.ESTADO AS ESTADO_FACTURA, 
       PFD.ID_SERVICIO, PFD.ID_PLAN, PFD.ID_FORMA_PAGO, PFD.STATUS_PAGO
FROM PAGOS_FACTURA_CLIENTE PFC
INNER JOIN PAGOS_FACTURA_CLIENTE_DETALLE PFD ON PFC.ID_PAGO = PFD.ID_PAGO;
GO
